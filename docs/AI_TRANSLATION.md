...
这份文档有一半会过期，另一半不会。判据是**每一条为什么会过期**：
...
| **外部政策约束**——不在权重里，在服务条款里 | 云端模型通常不被允许生成面向成人的露骨文本 | **不会过期**：条款不随模型变聪明而改变，所以"生成留在本机、编排留在云端"这个分工要一直保留（§2 §4.3） |
...
| 云端大模型直译 | 短篇、可以外发文本 | 文本离开本机；长文按 token 付费；**云端条款不允许它生成 18+ 内容**（这是下面那条分工的成因） |
...
**边界画在"谁产出"，不是"内容敏不敏感"。** 译文一律由**本机的本地模型**生成，云端 agent 不生成任何译文——它只做切分、术语表、提示词、校验、修复与构建。原因不是效率，是硬约束：主流云端模型的服务条款不允许它生成面向成人的露骨文本，而这类作品里成人向场景往往占大头（本项目 25 个 `Ｈシーン`、6 111 行）。所以别把"SFW 交云端、NSFW 交本地"当成设计：那条分界线不存在（日常场景也一样不外发），而且同一个作品分给两种模型译，术语与语气会对不上——§5 那句"同一事物全篇同一个词"只有在**同一侧**才管得住。
...
这条不是"选型偏好"，而是**架构约束**：云端模型不会生成这类文本，所以"生成"这一侧只能落在本机；换一个更强的云端模型也不改变这一点（§0 第 3 类）。也正因为这条约束来自服务条款而不是模型能力，它比本文档里任何 tok/s 数字都耐久。

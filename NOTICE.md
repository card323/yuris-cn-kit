...
| 2 | [yuris_decompiler](https://github.com/shimamura-sakura/yuris_decompiler)（MIT） | shimamura-sakura | 可以自由使用/再分发，但**本仓库不把它拷进来**（`.gitignore` 已排除），使用者自己 `git clone` |
...
* 对 yuris_decompiler 的其余依赖是**运行时调用**（`--repo` 指向一份 checkout），用于反编译 YSTB 与读取 `YSCM` 命令表。
...
| 发什么 | 说明 |
| --- | --- |
| 这套工具链 + 操作手册 | 最安全，用户自己抽自己的游戏。 |
| 「补丁器」脚本 | 让用户指向自己装的游戏目录，脚本读原版文件、转码、重新打包、安装。见 [docs/PIPELINE.md](docs/PIPELINE.md) §9。 |
| 你自己生成的 `update1.ypf` | ⚠️ 边界模糊：它替换了原包里的引擎脚本与对白文本，属于**游戏资源的修改版衍生**。发行商通常默许（这是目前 galgame 汉化的惯例），但这是"惯例"，不是"授权"。若要发布，请附上免责声明，并说明它必须配自己合法购买的副本使用。 |
| 字体 | 见 §3。 |
...
翻译文本的著作权属于译者。本仓库**不包含任何译文**：`workpack/`、`translation/`、`build/`、`yuris_text_out/` 全在 `.gitignore` 里。把工具链发出去时，顺手检查一遍这几个目录没有被打包进去。
